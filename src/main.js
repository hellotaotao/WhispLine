...
const { createPermissionManager } = require("./permission-managers");
...
async function setupGlobalHotkeys() {
  try {
    ...
    if (process.platform === "darwin") {
      const hasPermission = await permissionManager.checkAccessibilityPermissions();
      if (!hasPermission) {
        return;
      }
    }
...
ipcMain.handle("check-microphone-permission", async () => {
  return permissionManager.getMicrophonePermissionStatus();
});

ipcMain.handle("request-microphone-permission", async () => {
  // For system-level microphone permission, delegate to permission manager
  const hasPermission = await permissionManager.checkAndRequestMicrophonePermission();
...
ipcMain.handle("request-accessibility-permission", async () => {
  const result = await permissionManager.requestAccessibilityPermission();
  
  // Update our cached state
  if (result.granted) {
    currentAccessibilityPermission = result.granted;
  }
...
ipcMain.handle("recheck-accessibility-permission", async () => {
  ...
  const result = await permissionManager.recheckAccessibilityPermission();
  
  // Update cached state if permission changed
  if (result.changed) {
    currentAccessibilityPermission = result.granted;
  }
