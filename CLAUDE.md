...
The mac build scripts set `CI=true` (tauri's Finder-prettifying AppleScript fails in
non-interactive shells) and run `scripts/collect-artifacts.js`, which **always copies the built
`.dmg` into `dist/`**, the kept archive of every version's installer, so don't skip it.
...
**Local code signing:** the mac build scripts source an untracked `scripts/sign.env` (copy from
`scripts/sign.env.example`) exporting `APPLE_SIGNING_IDENTITY`. A stable identity keeps macOS's
Accessibility/Microphone grants across rebuilds. Ad-hoc signing (the default without the file)
changes the cdhash every build, which silently drops the Accessibility grant and with it the global
hotkey. Local builds are not notarized.

## Releasing

Push a `vX.Y.Z` tag. `.github/workflows/release.yml` builds macOS (universal, signed + notarized),
Windows and Linux, emits minisign-signed updater artifacts plus `latest.json`, and **publishes the
release directly** (`releaseDraft: false`). Publishing is the auto-update rollout: installed clients
poll `releases/latest/download/latest.json`. The workflow builds the version in the tagged commit,
so bump and commit first:
...
- `lib.rs` — builds the app: manages `AppState`; on `setup` creates the tray, reads config, checks
  ...
  **injects the per-window entry script** (`main.js` / `input-prompt.js`) on page load; registers
  all `#[tauri::command]` handlers; configures logging.
- `commands.rs` — all Tauri commands: settings, window control, `transcribe_audio` (routes to local
  ...
  - **Recovery invariants** (reasons and the complete rules: `docs/dictation-recovery.md`):
    - A failed transcription writes **one** History row carrying both reason and clip
      (`record_failed_transcription`, `pending: true`), in release builds too. Chunked dictation and
      local non-translation `capture_incomplete` are owned by the frontend instead.
...
- `local_asr.rs` — local Qwen3-ASR (0.6B default, 1.7B experimental) through upstream llama.cpp
  ...
  - The language setting and dictionary don't apply locally, and translation never runs locally.
...
- `nemotron_asr.rs` — Nemotron 3.5 streaming engine (NVIDIA `nemo-speech` sidecar), available only
  on macOS arm64 and Windows x64 (`supported()`).
...
- `platform/` — the platform abstraction (`mod.rs` contract + `macos.rs` / `fallback.rs`, plus the
  macOS-only `activation.rs` and `drag_cloud.rs`). Every `#[cfg(target_os)]` capability lives here:
  text insertion, permission checks, clipboard write, autostart. Non-macOS (`fallback.rs`, shared by
  Windows/Linux): insertion via `enigo`, permission checks report "not required", clipboard write and
  autostart are stubs. See `docs/superpowers/specs/2026-07-01-cross-platform-support-design.md`.
...
its registration in the `invoke_handler!` list in `lib.rs`, and the `tauriCommands` map in
...
and `emit_to` both work here. An earlier version of this file claimed `emit_to` was silently
dropped; that was wrong, so don't "fix" a working `emit_to` on its authority. `emit` is still the
natural default for events more than one window may want.
...
- Hold `Ctrl+Shift` to record and `Shift+Alt` to translate (cloud only, explicit provider plus
  consent); Escape cancels.
